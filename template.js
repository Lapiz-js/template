Lapiz.Module("Template", ["Collections"], function($L){
  ...
  self.set("Std", $L.Namespace(function Std(){
    ...
    this.meth(function tokenizer(template, ctx, resolver){
    ...
    this.meth(function resolver(token, ctx){
      if (token === "$") { return ctx; }
      var val, l, i;
      token = token.split(".");
      l = token.length;
    ...
    this.set("templator", $L.Template.Templator(this.namespace.tokenizer, this.namespace.resolver) );
